                ...
                if op == 0:
                    ...
                    if len(qs) > 0:
                        ...
                        if q.rdtype == dns.rdatatype.A:
                            ...
                            resp, dosleep,blocked = std_A_qry(msg, prov_ip)
                            #change printing only A files
                            #change task3: edited std_A_qry function and whichever modules uses domains and spoof gets True flag for blocking and other returns False
                            if not blocked:
                                save_req(LOGREQFILE, 'Client IP: ' + address[0] + '    request is    ' + str(q) + '\n')
                            else:
                                print "AAAAAAXXXXXXXXXXXXXXXXXXXAAAAAAAAAAAAAAAA"
