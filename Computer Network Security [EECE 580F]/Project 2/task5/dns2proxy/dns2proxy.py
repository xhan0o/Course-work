                ...
                if op == 0:
                    ...
                    if len(qs) > 0:
                        ...
                        if q.rdtype == dns.rdatatype.A:
                            ...
                            resp, dosleep,temp = std_A_qry(msg, prov_ip)
                            #change considering only A requests
                            #temp is storing ip addresses from std_A_qry function
                            save_req(LOGREQFILE, 'Client IP: ' + address[0] + '    request is    ' + str(q) + " " + str(temp) + '\n')
                            ...
                            resp,temp = std_AAAA_qry(msg)
                            #change using same logic for printing ipv6 also, But not implemented because of simplycity
...
def std_AAAA_qry(msg):
    ...
    if not Forward:
        DEBUGLOG('No host....')
        temp="no host" #Change handling exception when there are no host
        resp = make_response(qry=msg, RCODE=3)  # RCODE =  3	NXDOMAIN
        return resp,temp
    qs = msg.question
    DEBUGLOG(str(len(qs)) + ' questions.')
    ...
    for host in hosts:
        DEBUGLOG('Adding ' + host.to_text())
        temp=host.to_text() #change storing ipv6 in temp variable
...
def std_A_qry(msg, prov_ip):
    ...
    for q in qs:
        ...
        for realip in ips:
            ...
            temp=realip.to_text() #change storing ipv4 in temp
            #save_req(LOGREQFILE, realip.to_text())
            rrset = dns.rrset.from_text(q.name, ttl, dns.rdataclass.IN, dns.rdatatype.A, realip.to_text())
            resp.answer.append(rrset)

    return resp, dosleep,temp #returning temp variable
